...
/**
 * Datapoint BackboneJS model for IOSiX Fleet Webapp
 *
...
// Each Datapoint object corresponds to exactly on Datapoint record in the DB.
// Each database record corresponds to a single GPS location fix.
// Refer the toplevel README.md for a description of how the data packets are
// constructed.
...
var Datapoint = Backbone.RelationalModel.extend({
	...
	defaults: {
		// Latitude value of the data packet
		lat: 0.0,

		// Longitude value of the data packet
		lon: 0.0,

		// Timestamp of the data packet
		timestamp: 0,

		// Throttle position value of the data packet
		throttle: 0.0,

		// RPM value of the data packet
		rpm: 0.0,

		// Fuel level value of the data packet
		fuel: 0.0,

		// Mass Air Flow value of the data packet
		maf: 0.0
	},
...
// This is needed for the Trip -> Datapoint relationship
...
/**
 * Trip BackboneJS model for IOSiX Fleet Webapp
 *
 * Copyright 2014 IOSiX LLC
 * All Rights Reserved
 *
...
var Trip = Backbone.RelationalModel.extend({
	...
	// A 'Trip' has many 'Datapoint' child nodes
	...
	relations: [{
		...
		// When serializing a 'Trip' object into a JSON representation, be aware
	...
	defaults: {
		// Unique identifier of each Trip object
		id: "",

		// A key to a "Vehicle" object unique identifier
		vehicle_id: "",

		// If truthy, the the Datapoint objects that are children of
		// this Trip object will be shown on the map.
		display: false,
 
		// A date-time string that corresponds to the earliest
		// timestamp present in all Datapoint child objects of
		// this Trip
		start: "",

		// A date-time string that corresponds to the latest
		// timestamp present in all Datapoint child objects of
		// this Trip
		end: ""
	},

	// A URI fragment that corresponds a the REST API's URL
	// of the endpoint for Trip data objects
	urlRoot: '/trips/'
});

// Definition for a Backbone Collection of Trip models
// This is (probably) needed for the Vehicle->Trip relationship to work
// But if it is not needed for that purpose, it makes a group of Trip 
...
/**
 * User BackboneJS model for IOSiX Fleet Webapp
 *
 * Copyright 2014 IOSiX LLC
 * All Rights Reserved
 *
...
/**
 * Vehicle BackboneJS model for IOSiX Fleet Webapp
 *
 * Copyright 2014 IOSiX LLC
 * All Rights Reserved 
 *
...
var Vehicle = Backbone.RelationalModel.extend({
	idAttribute: 'id',
	
	// A 'Vehicle' has many 'Trip' children nodes
	// This "relation" key (and related functionality) comes from the
	// 'backbone-relational.js' library.
	relations: [{
		type: Backbone.HasMany,
		...
		reverseRelation: {
			...
			// you try to serialize a "Vehicle" object with children to
	...
	defaults: {
		...
		// Vehicle Vin
		// Currently, the vehicle's VIN must be manually
		// entered when adding the vehicle. This may need
		// to be changed in the future.
		vin: "ABCDEFGHJKLMNPRST",

		// if truthy, then the vehicle's Trip objects
		// (if any) will get displayed
		display: false
	},

	// A URI fragment that corresponds to the REST
	// API's URL for the Vehicle data objects.
	// Because 'idAttribute' is defined as the 'id'
	// attribute, the URL for a Vehicle backbone object
	// is "/vehicles/<id>"
	urlRoot: '/vehicles/'
});

// Definition for a Backbone Collection of Vehicle models
...
var AppView = Backbone.View.extend({
	...
	events: {
		...
		'click .add-vehicle-button': 'showAddVehicleModal',
	...
	showAddVehicleModal: function(e) {
		this.$("#modal-background").show();
		this.$("#add-vehicle-modal").show();
	},
	...
	initialize: function(options) {
		...
		// info.
		this.listenTo(this.vehicles, 'update change', this.populateMap);
		...
		this.listenTo(this.vehicles, 'add remove', this.render);
		...
		// Make sure that our collection of Vehicle objects is up to date.
		this.vehicles.fetch();
	...
	addOneVehicle: function(vehicle) {
	...
	addAllVehicles: function() {
		this.vehicles.each(this.addOneVehicle, this);
	},
...
var MAKE = ["Chevrolet", "Ford", "GM", "Chrysler", "Toyota"];
